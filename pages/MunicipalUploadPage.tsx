...
const MunicipalUploadPage: React.FC<MunicipalUploadPageProps> = ({ voters, onDecommission }) => {
  ...
  const [extractedData, setExtractedData] = useState<{ name: string, aadhaarNumber?: string, dateOfDeath?: string } | null>(null);
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        if (result && result.name) {
          ...
          // PROTOCOL: Identify node by Aadhaar match or Name + Hash match
          const match = voters.find(v => {
            // Check if Aadhaar hash matches extracted number (simulating hash matching)
            const aadhaarMatches = result.aadhaarNumber && v.aadhaarMeta?.aadhaarIdHash.includes(result.aadhaarNumber.split('-').pop() || 'NONE');
            const nameMatches = v.name.toLowerCase() === result.name.toLowerCase();
            return aadhaarMatches || nameMatches;
          });
  ...
  const finalizeDecommission = () => {
    ...
    onDecommission(matchedVoter.id, 'Deceased (Verified via Aadhaar-linked Certificate)');
    alert(`Protocol Success: Voter ${matchedVoter.id} (Aadhaar: ${matchedVoter.aadhaarMeta?.aadhaarIdHash}) removed from active roll.`);
    setFile(null);
    setExtractedData(null);
    setMatchedVoter(null);
  };
    ...
    <div className="space-y-8 max-w-4xl mx-auto">
      <div className="border-b-4 border-slate-900 pb-6">
        <h1 className="text-2xl font-black text-slate-900 uppercase tracking-tighter flex items-center gap-3">
          ...
          Civil Registry: Aadhaar Death Verification
        </h1>
        <p className="text-slate-500 text-sm font-bold uppercase tracking-widest mt-2">Municipal Corporation Portal • Identity-Based Decommissioning</p>
      </div>
      ...
      <div className="bg-white border-2 border-slate-900 p-8 shadow-[8px_8px_0px_0px_rgba(15,23,42,1)]">
        <div className="space-y-6">
          ...
          <button
            ...
            className={`w-full py-4 text-xs font-black uppercase tracking-[0.4em] transition-all flex items-center justify-center gap-3 ${!file || isProcessing ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-slate-900 text-white hover:bg-blue-600'
              }`}
          >
            {isProcessing ? "Verifying Aadhaar Records..." : "Submit for Identity Validation"}
          </button>
        </div>
      </div>
      ...
      {extractedData && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="bg-slate-900 text-white p-6 border-t-8 border-blue-500">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-[9px] font-bold text-slate-500 uppercase">Aadhaar Number</p>
                  <p className="font-mono text-sm">{extractedData.aadhaarNumber || 'N/A'}</p>
                </div>
                <div>
          ...
          <div className={`p-6 border-2 ${matchedVoter ? 'bg-orange-50 border-orange-600' : 'bg-slate-100 border-slate-300 opacity-60'}`}>
            <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-4">Aadhaar Identity Match</h3>
            {matchedVoter ? (
              <div className="space-y-6">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] font-mono font-bold text-orange-600 mt-1">
                      EPIC: {matchedVoter.id}
                    </p>
                    <p className="text-[9px] font-mono font-bold text-slate-500 mt-0.5">
                      AADHAAR HASH: {matchedVoter.aadhaarMeta?.aadhaarIdHash}
                    </p>
                ...
                <div className="p-4 bg-white/80 border border-orange-200 text-[10px] font-black text-orange-900 uppercase">
                  IDENTITY VERIFIED: ADHAAR LINK DETECTED
                </div>
                ...
                >
                  <UserMinus className="w-4 h-4" /> Decommission Identity
                </button>
              </div>
            ) : (
              <div className="h-40 flex flex-col items-center justify-center text-center">
                <Search className="w-8 h-8 text-slate-300 mb-2" />
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">No matching Aadhaar profile detected.</p>
              </div>
            )}
          </div>
        </div>
